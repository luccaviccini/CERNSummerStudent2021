...
variable tbLegacyEnt init_ram_legacy_tb
variable tb2008Ent init_ram_2008_tb
variable projLibs {
  work
}
variable runTime "100 ns"
...
proc runlegacy {} {
  global tbLegacyEnt runTime
  vsim -onfinish stop work.$tbLegacyEnt
  if {[file exists wave.do]} {
    do wave.do
  }
  run $runTime
  echo "VHDL-93 testbench done, check waveform"
}

proc run2008 {} {
  global tb2008Ent runTime
  vsim -onfinish stop work.$tb2008Ent
  if {[file exists wave.do]} {
    do wave.do
  }
  run $runTime
  echo "VHDL-2008 testbench done, check waveform"
}
...
echo "*                                                             *"
echo "* Type \"run2008\" in the ModelSim console to run the testbench *"
echo "* for VHDL-2008 or \"runlegacy\" to run the test for VHDL-93    *"
echo "*                                                             *"
echo "* Like this:                                                  *"
echo "*                                                             *"
echo "* ModelSim> run2008                                           *"
echo "* or                                                          *"
echo "* ModelSim> runlegacy                                         *"
echo "*                                                             *"
echo "***************************************************************"